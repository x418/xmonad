...
This package builds against river/Wayland and nothing else. Its API must remain
a strict subset of upstream xmonad's: `tests/api/upstream/` is upstream's own
interface, recorded from a real checkout with
...
**Status: a daily-driven desktop.**  Multi-monitor, workspaces, floats and
drags, prompts, decorations, EasyMotion, scratchpads, restart with state.
304 of xmonad-contrib's 334 modules build; see `SURVEY.md` there.  The loop
and worker design is in `DESIGN.md`; what remains to do is in `REFACTOR.md`.
...
`src/` is this backend's source and the only source in the tree. The X11 build
is not here at all; the thing to diff against is an upstream xmonad checkout,
which is what `../xmonad` means throughout this document and in
`tests/api/dump-api.sh`.

Nothing in `src/` is named `Graphics.X11`.

Every module under `src/XMonad/` other than `XMonad.River.*` is a copy of the
upstream module of the same name, and each stands in one of three relations to
its original:

| | | |
| --- | --- | --- |
| **identical** | `XMonad.StackSet` | byte-for-byte. Pure, total, mentions neither backend — any difference at all is a bug |
| **patched** | `XMonad`, `XMonad.Core`, `XMonad.Layout` | a small tracked divergence; the bulk of the file is upstream's and should stay that way |
| **rewritten** | `XMonad.Config`, `.ManageHook`, `.Main`, `.Operations` | different programs that agree on an interface |

The patched category is where rebase safety lives. `XMonad.Core` is ~84% shared
with upstream — it holds `recompile`, `getDirectories` and the whole compile
path, none of which has anything to do with X11 — so an upstream fix there
should carry over rather than be discovered months later as fixed in one copy
only. Keeping the shared bulk byte-identical to upstream, and the divergence
small and deliberate, is what makes that possible.

The categories above are a description, not an assertion.
`tests/api/check-subset.sh` pins the exported API against upstream's, but
nothing currently checks module bodies for drift.
